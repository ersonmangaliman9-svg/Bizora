...
/* ---------------- Manual subscription backend ----------------
   Bizora has no real server, so "unlocking Premium" is done manually:
   1. A user registers -> a record is saved to SHARED storage under
      `user:<email>` so the owner (admin) can see everyone who signed up.
   2. A user taps Subscribe -> a request is saved to SHARED storage under
      `subrequest:<email>` with status "pending", and the user is told to
      message the admin to confirm their payment.
   3. Only the admin account (matched by ADMIN_EMAIL) can see the hidden
      Admin panel, where they Approve/Reject requests. Approving flips
      `isPremium` on that user's `user:<email>` record, and every signed-in
      device quietly polls that record to pick up the change.
--------------------------------------------------------------- */
...
const userRecordKey = (email) => `user:${(email || "").trim().toLowerCase()}`;
const subRequestKey = (email) => `subrequest:${(email || "").trim().toLowerCase()}`;
...
const seedData = () => {
  ...
  // new store already has its whole product line in Inventory from day one.
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    sales.push({
      ...
      unitPrice: p.sellingPrice, total: qty * p.sellingPrice,
      customerId: customers[Math.floor(Math.random() * customers.length)].id,
      payment: PAYMENT_METHODS[Math.floor(Math.random() * PAYMENT_METHODS.length)].id,
      notes: "", invoiced: false,
    });
    if (Math.random() > 0.4) {
      expenses.push({ id: uid(), date: iso, category: "Supplies", amount: 50 + Math.floor(Math.random() * 300), notes: "Restock" });
    }
  }
...
function SplashScreen({ onDone }) {
  ...
  return (
    <div onClick={skip} style={{
      height: 700, maxWidth: 420, margin: "0 auto",
      background: "radial-gradient(circle at 50% 42%, #0F7A63 0%, #0A5548 55%, #072F28 100%)",
      display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
      cursor: "pointer", overflow: "hidden", position: "relative", borderRadius: 28,
...
export default function Bizora() {
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        await window.storage.set(STORAGE_KEY, JSON.stringify({
          authed, onboarded, authUser, theme, isPremium, notifyLowStock, business, products, customers, sales, expenses, invoices,
        }), false);
      } catch (e) { /* silent */ }
    }, 400);
  }, [loaded, authed, onboarded, authUser, theme, isPremium, notifyLowStock, business, products, customers, sales, expenses, invoices]);

  const showToast = (msg) => { setToast(msg); setTimeout(() => setToast(null), 2200); };

  // Register / refresh this user's record in SHARED storage so the admin
  // can see who has registered, and so Premium status can be granted
  // remotely by the admin and picked up here.
  useEffect(() => {
    if (!loaded || !authed || !authUser?.email) return;
    (async () => {
      const key = userRecordKey(authUser.email);
      const existing = await getSharedJSON(key);
      const record = {
        name: authUser.name || existing?.name || "",
        email: authUser.email,
        businessName: business?.name || existing?.businessName || "",
        businessType: business?.type || existing?.businessType || "",
        avatar: business?.avatar || existing?.avatar || defaultAvatarPreset(authUser.email),
        registeredAt: existing?.registeredAt || new Date().toISOString(),
        isPremium: existing ? !!existing.isPremium : !!isPremium,
      };
      await setSharedJSON(key, record);
      // If the admin already flipped this account to Premium in a previous
      // session, reflect that locally right away.
      if (existing?.isPremium && !isPremium) {
        lastKnownPremiumRef.current = true;
        setIsPremium(true);
      } else {
        lastKnownPremiumRef.current = !!isPremium;
      }
    })();
  }, [loaded, authed, authUser?.email, authUser?.name, business?.name, business?.type]);

  // Poll every ~15s for admin approval of a pending subscription request,
  // since there's no real backend to push the update instantly.
  useEffect(() => {
    if (!loaded || !authed || !authUser?.email) return;
    const check = async () => {
      const rec = await getSharedJSON(userRecordKey(authUser.email));
      if (rec && !!rec.isPremium !== lastKnownPremiumRef.current) {
        lastKnownPremiumRef.current = !!rec.isPremium;
        setIsPremium(!!rec.isPremium);
        if (rec.isPremium) showToast("You've been approved — Bizora Premium unlocked ✨");
      }
    };
    premiumPollRef.current = setInterval(check, 15000);
    return () => clearInterval(premiumPollRef.current);
  }, [loaded, authed, authUser?.email]);
  ...
  if (!loaded) {
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 12, alignItems: "center", justifyContent: "center", height: 700, maxWidth: 420, margin: "0 auto", borderRadius: 28, overflow: "hidden", background: LIGHT.bg, fontFamily: "'Sora', sans-serif" }}>
  ...
  if (!authed) {
    return (
      <AuthFlow
        ...
        theme={theme}
        authUser={authUser}
        onSignedIn={(user) => { setAuthUser(user); setAuthed(true); showToast(`Welcome back, ${user.name.split(" ")[0]}!`); }}
        onSignedUp={(user) => { setAuthUser(user); setAuthScreen("onboarding"); }}
        onOnboardingDone={(biz) => { setBusiness({ avatar: defaultAvatarPreset(authUser?.email || biz.owner), ...biz }); setOnboarded(true); setAuthed(true); showToast("Account created ✓"); }}
      />
    );
  }
  ...
  if (!business) {
    return (
      <AuthFlow
        ...
        onSignedIn={() => {}}
        onSignedUp={() => {}}
        onOnboardingDone={(biz) => { setBusiness({ avatar: defaultAvatarPreset(authUser?.email || biz.owner), ...biz }); setOnboarded(true); showToast(`Set up ✓ Welcome, ${(authUser?.name || biz.owner || "").split(" ")[0] || "there"}!`); }}
      />
    );
  }
  ...
  return (
    <div style={{
      ...
      maxWidth: 420, margin: "0 auto", minHeight: 700, position: "relative",
      borderRadius: 28, overflow: "hidden", boxShadow: theme === "dark" ? "0 0 0 1px #1c2622" : "0 0 0 1px #eae6da",
    }}>
      <style>{`
        ...
        .bz-input { background: var(--surface2); border: 1px solid var(--border); border-radius: 12px; color: var(--text); padding: 11px 13px; font-size: 14px; width: 100%; font-family: 'Inter', sans-serif; transition: border-color .15s ease; }
      ...
      <div ref={scrollRef} className="bz-scroll" style={{ height: 700, overflowY: "auto", paddingBottom: 88 }}
        ...
        {tab === "more" && moreScreen === "settings" && (
          <SettingsScreen {...{ business, setBusiness, theme, setTheme, colors, isPremium, authUser, onBack: () => setMoreScreen(null), goUpgrade: () => setMoreScreen("upgrade"),
            onLogout: () => { setAuthed(false); setAuthScreen("signin"); }, showToast, notifyLowStock, setNotifyLowStock }} />
        )}
        {tab === "more" && moreScreen === "upgrade" && (
          <UpgradeScreen {...{ colors, isPremium, authUser, business,
            onBack: () => setMoreScreen(null), goSettings: () => setMoreScreen("settings"), showToast }} />
        )}
        {tab === "more" && moreScreen === "admin" && isAdmin && (
          <AdminScreen {...{ colors, onBack: () => setMoreScreen(null), showToast }} />
        )}
      </div>
...
function InventoryScreen({ products, colors, isPremium, onAdd, onEdit, onDelete, goUpgrade }) {
  const [filter, setFilter] = useState("");
...
function SettingsScreen({ business, setBusiness, theme, setTheme, colors, isPremium, authUser, onBack, goUpgrade, onLogout, showToast, notifyLowStock, setNotifyLowStock }) {
...
function UpgradeScreen({ colors, isPremium, authUser, business, onBack, goSettings, showToast }) {
  ...
  useEffect(() => {
    let alive = true;
    if (!authUser?.email) { setPendingRequest(null); return; }
    getSharedJSON(subRequestKey(authUser.email)).then(r => { if (alive) setPendingRequest(r); });
    return () => { alive = false; };
  }, [authUser?.email]);

  const submitRequest = async () => {
    if (!authUser?.email) { showToast("Please sign in again."); return; }
    setSubmitting(true);
    const request = {
      name: authUser.name || "", email: authUser.email,
      businessName: business?.name || "", cycle, price, refNo: refNo.trim(),
      requestedAt: new Date().toISOString(), status: "pending",
    };
    await setSharedJSON(subRequestKey(authUser.email), request);
...
function AdminScreen({ colors, onBack, showToast }) {
  ...
  const loadAll = async () => {
    ...
    try {
      const [reqList, userList] = await Promise.all([
        window.storage.list("subrequest:", true),
        window.storage.list("user:", true),
      ]);
      const reqs = await Promise.all((reqList?.keys || []).map(k => getSharedJSON(k)));
      const usrs = await Promise.all((userList?.keys || []).map(k => getSharedJSON(k)));
      setRequests(reqs.filter(Boolean).sort((a, b) => (b.requestedAt || "").localeCompare(a.requestedAt || "")));
      setUsers(usrs.filter(Boolean).sort((a, b) => (b.registeredAt || "").localeCompare(a.registeredAt || "")));
    } catch (e) {
  ...
  const decide = async (req, approve) => {
    setBusyKey(req.email);
    try {
      const uKey = userRecordKey(req.email);
      const rKey = subRequestKey(req.email);
      const updatedReq = { ...req, status: approve ? "approved" : "rejected", decidedAt: new Date().toISOString() };
      await setSharedJSON(rKey, updatedReq);
      if (approve) {
        const existingUser = await getSharedJSON(uKey);
        await setSharedJSON(uKey, { ...(existingUser || { email: req.email, name: req.name, businessName: req.businessName, registeredAt: new Date().toISOString() }), isPremium: true });
      }
  ...
  const togglePremium = async (u) => {
    setBusyKey(u.email);
    try {
      const key = userRecordKey(u.email);
      const existing = await getSharedJSON(key);
      await setSharedJSON(key, { ...(existing || u), isPremium: !u.isPremium });
      showToast(!u.isPremium ? `Premium granted to ${u.email}` : `Premium revoked from ${u.email}`);
      await loadAll();
    } catch (e) {
      showToast("Something went wrong, try again");
    } finally {
      ...
      <div style={{ padding: "6px 18px 40px" }}>
        ...
        {tabView === "requests" && (
          requests === null ? (
            ...
            <>
              ...
              {pending.map(r => (
                <div key={r.email} className="bz-card" style={{ padding: "13px 14px", marginBottom: 9 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 13.5 }}>{r.name || "—"}</div>
                      <div style={{ fontSize: 11.5, color: "var(--muted)" }}>{r.email}</div>
                      {r.businessName && <div style={{ fontSize: 11, color: "var(--muted)" }}>{r.businessName}</div>}
                    </div>
                  ...
                  {r.refNo && <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 6 }}>Ref: {r.refNo}</div>}
                  <div style={{ fontSize: 10.5, color: "var(--muted)", marginTop: 4 }}>{r.requestedAt ? new Date(r.requestedAt).toLocaleString("en-PH") : ""}</div>
                  <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                    <button disabled={busyKey === r.email} onClick={() => decide(r, true)} className="bz-btn-primary"
                      style={{ flex: 1, padding: "9px", fontSize: 12.5, display: "flex", alignItems: "center", justifyContent: "center", gap: 5, opacity: busyKey === r.email ? 0.6 : 1 }}>
                      <Check size={13} /> Approve
                    </button>
                    <button disabled={busyKey === r.email} onClick={() => decide(r, false)}
                      style={{ flex: 1, padding: "9px", fontSize: 12.5, borderRadius: 14, border: "1px solid var(--border)", background: "var(--surface)", color: "var(--danger)", fontWeight: 600, cursor: "pointer", opacity: busyKey === r.email ? 0.6 : 1 }}>
                      Reject
                    </button>
                  </div>
                </div>
              ...
              {decided.map(r => (
                <div key={r.email + r.decidedAt} className="bz-card" style={{ padding: "11px 14px", marginBottom: 8, display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ flex: 1 }}>
        ...
        {tabView === "users" && (
          users === null ? (
            ...
            <div key={u.email} className="bz-card" style={{ padding: "12px 14px", marginBottom: 8, display: "flex", alignItems: "center", gap: 10 }}>
              <ProfileAvatar avatar={u.avatar} size={38} fallback={(u.name || u.email || "?").charAt(0).toUpperCase()} />
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 700, fontSize: 13 }}>{u.name || "—"}</div>
                <div style={{ fontSize: 11, color: "var(--muted)" }}>{u.email}</div>
                {u.businessName && <div style={{ fontSize: 10.5, color: "var(--muted)" }}>{u.businessName}</div>}
                <div style={{ fontSize: 10, color: "var(--muted)", marginTop: 2 }}>Registered {u.registeredAt ? new Date(u.registeredAt).toLocaleDateString("en-PH") : "—"}</div>
              </div>
              {u.isPremium && <PremiumBadge />}
              <button disabled={busyKey === u.email} onClick={() => togglePremium(u)} style={{
                fontSize: 10.5, fontWeight: 700, padding: "4px 9px", borderRadius: 999, cursor: "pointer",
                border: u.isPremium ? "1px solid var(--danger)" : "1px solid var(--primary)",
                background: "var(--surface)", color: u.isPremium ? "var(--danger)" : "var(--primary)",
                opacity: busyKey === u.email ? 0.5 : 1,
              }}>
...
function AuthFlow({ screen, setScreen, theme, authUser, onSignedIn, onSignedUp, onOnboardingDone }) {
  ...
  return (
    <div style={{
      ...
      "--danger": colors.danger, "--success": colors.success,
      background: "var(--bg)", color: "var(--text)", fontFamily: "'Inter', sans-serif",
      maxWidth: 420, margin: "0 auto", minHeight: 700, height: 700, position: "relative",
      borderRadius: 28, overflow: "hidden", boxShadow: theme === "dark" ? "0 0 0 1px #1c2622" : "0 0 0 1px #eae6da",
    }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Sora:wght@400;600;700;800&family=Inter:wght@400;500;600;700&display=swap');
        ...
        .bz-input { background: var(--surface2); border: 1px solid var(--border); border-radius: 12px; color: var(--text); padding: 11px 13px; font-size: 14px; width: 100%; font-family: 'Inter', sans-serif; }
        .bz-input:focus { outline: none; border-color: var(--primary); }
        .bz-fade-in { animation: bzfade .18s ease; }
        @keyframes bzfade { from { opacity: 0; transform: translateY(6px); } to { opacity: 1; transform: translateY(0); } }
        * { box-sizing: border-box; }
...
function SignInScreen({ colors, onSignedIn, goSignUp, goForgot }) {
  ...
  const submit = () => {
    ...
    onSignedIn({ name: name || "Business Owner", email });
  };
  ...
  return (
    <div>
      ...
      <div style={{ padding: "10px 26px 40px" }}>
        ...
        <button className="bz-btn-primary" style={{ width: "100%", padding: "13px", fontSize: 14 }} onClick={submit}>Log In</button>
        <Divider />
        <GoogleButton label="Continue with Google" onClick={() => onSignedIn({ name: "Business Owner", email: "owner@gmail.com" })} />
  ...
  return (
    <div>
      ...
      <div style={{ padding: "10px 26px 40px" }}>
        ...
        {error && <div style={{ fontSize: 12, color: "var(--danger)", marginBottom: 12 }}>{error}</div>}
        <button className="bz-btn-primary" style={{ width: "100%", padding: "13px", fontSize: 14 }} onClick={submit}>Create Account</button>
        <Divider />
        <GoogleButton label="Sign up with Google" onClick={() => onSignedUp({ name: "Business Owner", email: "owner@gmail.com" })} />
        <div style={{ textAlign: "center", fontSize: 12.5, color: "var(--muted)", marginTop: 22 }}>
