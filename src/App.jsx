  ...
  if (!authed) {
    return (
      <AuthFlow
        ...
        onOnboardingDone={(biz) => { setBusiness(biz); setOnboarded(true); setAuthed(true); showToast("Account created ✓"); }}
      />
    );
  }
  ...
  if (!business) {
    return (
      <AuthFlow
        ...
        onSignedIn={() => {}}
        onSignedUp={() => {}}
        onOnboardingDone={(biz) => { setBusiness(biz); setOnboarded(true); showToast(`Set up ✓ Welcome, ${(authUser?.name || biz.owner || "").split(" ")[0] || "there"}!`); }}
      />
    );
  }
        ...
        {tab === "more" && moreScreen === "upgrade" && (
          <UpgradeScreen {...{ colors, isPremium, authUser, business,
            ...
            onCancel: () => {
              ...
              if (authUser?.email) {
                getSharedJSON(userRecordKey(authUser.email)).then(existing => {
                  setSharedJSON(userRecordKey(authUser.email), { ...(existing || {}), isPremium: false });
                });
              }
            } }} />
        )}
...
function UpgradeScreen({ colors, isPremium, authUser, business, onBack, goSettings, showToast, onCancel }) {
    ...
    return (
      <div>
        ...
        <div style={{ padding: "10px 18px" }}>
          ...
          <button onClick={() => { onCancel(); showToast("Subscription cancelled"); }} style={{ width: "100%", marginTop: 12, padding: "11px", borderRadius: 14, border: "1px solid var(--border)", background: "var(--surface)", color: "var(--danger)", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
            Cancel Subscription
          </button>
        </div>
      </div>
        ...
        {tabView === "users" && (
          users === null ? (
            ...
            <div key={u.email} className="bz-card" style={{ padding: "12px 14px", marginBottom: 8, display: "flex", alignItems: "center", gap: 10 }}>
              <ProfileAvatar size={38} fallback={(u.name || u.email || "?").charAt(0).toUpperCase()} />
              <div style={{ flex: 1 }}>
