  ...
  return (
    <div style={{
      ...
      <div ref={scrollRef} className="bz-scroll" style={{ height: 700, overflowY: "auto", paddingBottom: 88 }}
        ...
        {tab === "more" && !moreScreen && (
          <MoreMenu {...{ colors, isPremium, setMoreScreen, business, authUser }} />
        )}
        ...
        {tab === "more" && moreScreen === "upgrade" && (
          <UpgradeScreen {...{ colors, isPremium, setIsPremium, business, onBack: () => setMoreScreen(null), goSettings: () => setMoreScreen("settings"), showToast }} />
        )}
      </div>
...
function MoreMenu({ colors, isPremium, setMoreScreen, business, authUser }) {
  const items = [
...
function UpgradeScreen({ colors, isPremium, setIsPremium, business, onBack, goSettings, showToast }) {
  const [cycle, setCycle] = useState("yearly");
  const [step, setStep] = useState("plan"); // "plan" | "pay"
  const [refNo, setRefNo] = useState("");
  const price = cycle === "monthly" ? 149 : 1199;
  const priceLabel = cycle === "monthly" ? "₱149" : "₱1,199";
    ...
    return (
      <div>
        ...
        <div style={{ padding: "10px 18px" }}>
          ...
          <button onClick={() => { setIsPremium(false); showToast("Subscription cancelled"); }} style={{ width: "100%", marginTop: 12, padding: "11px", borderRadius: 14, border: "1px solid var(--border)", background: "var(--surface)", color: "var(--danger)", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
            Cancel Subscription
          </button>
          <button onClick={() => showToast("You're already Premium ✓")} style={{ width: "100%", marginTop: 8, padding: "11px", borderRadius: 14, border: "none", background: "var(--surface2)", color: "var(--text)", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
            Restore Purchase
          </button>
        </div>
      </div>
    ...
    return (
      <div>
        ...
        <div style={{ padding: "6px 18px 30px" }}>
          ...
          <button onClick={() => { setIsPremium(true); showToast("Payment confirmed — Welcome to Bizora Premium ✨"); onBack(); }}
            className="bz-btn-primary" style={{ width: "100%", padding: "13px", fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}>
            <Check size={15} /> I've Sent the Payment — Activate Premium
          </button>
          <div style={{ textAlign: "center", fontSize: 10.5, color: "var(--muted)", marginTop: 8 }}>
            We'll confirm your {priceLabel} payment and unlock Premium instantly.
          </div>
        </div>
      </div>
    );
    ...
    <div>
      ...
      <div style={{ padding: "6px 18px" }}>
        ...
        <div style={{ display: "flex", gap: 8, margin: "14px 0" }}>
          <PlanOption active={cycle === "monthly"} onClick={() => setCycle("monthly")} title="Monthly" price="₱149/mo" note="Billed monthly" />
          <PlanOption active={cycle === "yearly"} onClick={() => setCycle("yearly")} title="Yearly" price="₱1,199/yr" note="Save 33% · ₱99/mo" badge="Best Value" />
        </div>
        ...
        <div style={{ textAlign: "center", fontSize: 11, color: "var(--muted)", marginTop: 8 }}>
          Then {cycle === "monthly" ? "₱149/month" : "₱1,199/year"}. Cancel anytime.
        </div>
        <button onClick={() => showToast("No previous purchase found")} style={{ width: "100%", marginTop: 10, padding: "9px", background: "none", border: "none", color: "var(--muted)", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
          Restore Purchase
        </button>
      </div>
      <div style={{ height: 20 }} />
