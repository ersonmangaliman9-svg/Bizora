...
function AdminScreen({ colors, sbToken, onBack, showToast }) {
  ...
  const decide = async (req, approve) => {
    ...
    try {
      ...
      if (approve) {
        await sbRest(`/profiles?id=eq.${req.user_id}`, { method: "PATCH", token: sbToken, body: { is_premium: true } });
      }
  ...
  const togglePremium = async (u) => {
    setBusyKey(u.email);
    try {
      await sbRest(`/profiles?id=eq.${u.id}`, { method: "PATCH", token: sbToken, body: { is_premium: !u.is_premium } });
        ...
        {tabView === "requests" && (
          requests === null ? (
            ...
            <>
              ...
              {decided.map(r => (
                <div key={r.email + r.decided_at} className="bz-card" style={{ padding: "11px 14px", marginBottom: 8, display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ flex: 1 }}>
